...
namespace database
{
    ...
    void Cache::put([[maybe_unused]] std::string login, [[maybe_unused]] const std::string &val)
    {
        std::lock_guard<std::mutex> lck(_mtx);
        rediscpp::value response = rediscpp::execute(*_stream, "set", login, val);
    }

    bool Cache::get([[maybe_unused]] std::string login, [[maybe_unused]] std::string &val)
    {
        std::lock_guard<std::mutex> lck(_mtx);
